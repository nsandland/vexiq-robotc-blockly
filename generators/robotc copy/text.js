...
Blockly.RobotC['text_join'] = function(block) {
  ...
  switch (block.itemCount_) {
    case 0:
      return ['EMPTY_STRING', Blockly.RobotC.ORDER_ATOMIC];
    default:
      var code = '';
      for (var i = block.itemCount_ - 1; i >= 0; i--) {
        var element = Blockly.RobotC.valueToCode(block, 'ADD' + i,
            Blockly.RobotC.ORDER_COMMA) || 'EMPTY_STRING';
        if (code.length == 0) {
          code = element;
        } else {
          code = 'vstrjoin(' + element + ', ' + code + ')';
        }
      }
...
Blockly.RobotC['text_append'] = function(block) {
  ...
  var value = Blockly.RobotC.valueToCode(block, 'TEXT',
      Blockly.RobotC.ORDER_NONE) || 'EMPTY_STRING';
  return 'vstrcat(' + varName + ', ' + value + ');\n';
};

Blockly.RobotC['text_length'] = function(block) {
  // String or array length.
  var text = Blockly.RobotC.valueToCode(block, 'VALUE',
      Blockly.RobotC.ORDER_FUNCTION_CALL) || 'EMPTY_STRING';
  return ['vstrlen(' + text + ')', Blockly.RobotC.ORDER_FUNCTION_CALL];
};

Blockly.RobotC['text_isEmpty'] = function(block) {
  // Is the string null or array empty?
  var text = Blockly.RobotC.valueToCode(block, 'VALUE',
      Blockly.RobotC.ORDER_MEMBER) || 'EMPTY_STRING';
  return ['vstrempty(' + text + ')', Blockly.RobotC.ORDER_FUNCTION_CALL];
};
...
Blockly.RobotC['text_reverse'] = function(block) {
  var text = Blockly.RobotC.valueToCode(block, 'TEXT',
      Blockly.RobotC.ORDER_MEMBER) || 'EMPTY_STRING';
  var code = text + '.split(EMPTY_STRING).reverse().join(EMPTY_STRING)';
  return [code, Blockly.RobotC.ORDER_MEMBER];
};
